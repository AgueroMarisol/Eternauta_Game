...
object juan inherits Personaje(
	...
	method crearAnimacionCaminar() = new JuanCaminando(image = self.image(), position = self.position())
	method posicion() = self.position()
    ...
    method causarDanioSiEsColisionado() {
    ...
    method moverIzquierda() {
        if (puedeMoverse) {
            ...
            self.causarDanioSiEsColisionado()
    ...
    method moverDerecha() {
        if (puedeMoverse) {
            position = game.at((position.x()+1).min(14), position.y())
            self.causarDanioSiEsColisionado()
    ...
    /* Simula la muerte del jugador y muestra la pantalla de Game Over */
    method morir() {
...
class Enemigo inherits Personaje(position = game.at(game.width(), [ 1, 3 ].anyOne()), esEnemigo = true) {
	...
	method desplazarse() {
		if (position.x() < -2) self.desaparecer() else position = position.left(1)
	}
	...
	override method serImpactado(objeto) {
		...
		if (vida == 0) {
			self.morirYSumarPuntos()
		}
	}

	/* Eliminar al enemigo y otorgar puntos por su muerte */
	method morirYSumarPuntos() {
		self.desaparecer()
		puntuacion.sumarPuntos(self)
	}

	/* Método vacío para ser sobrescrito por subclases */
	method morir() // Llevarlo a Personaje
...
class HombreRobot inherits Enemigo(image = "hombresRobots/Hombre_Robot_Base-272x192.png",
	...
	method generarEnemigos() {
		game.onTick(3000, "generar_hombres_robots", { const nuevoHombreRobot = new HombreRobot()
...
/* 	/* Cambiar la imagen del Cascarudo */
	...
	override method movimiento() {
		game.onTick((700..900).anyOne(), id + "_movimiento", { self.desplazarse()})
...
object mano inherits Personaje(position = game.at(18, 2), image = "Botiquin_112x50.png", vida = vidaEscudoRayo.vida()) {

	var property tiempoAtaque = 3000
	const property armaTerrenal = [ rayoTerrenal1, rayoTerrenal2 ]
